...
> Template-Engine - Open source tool by AetherCodeHQ

`cli` `developer-tools` `engine` `golang` `templating`

## What is it?

**Template Engine** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Template-Engine --path ./target

# With options
./Template-Engine --path ./target --format json --output report.json

# Verbose mode
./Template-Engine --path ./target --verbose
```
...
### Basic scan
```bash
./Template-Engine --path ./my-project
```

### JSON report
```bash
./Template-Engine --path ./my-project --format json --output report.json
```
...
- name: Run Template Engine
  run: ./Template-Engine --path . --format json --output report.json
```
